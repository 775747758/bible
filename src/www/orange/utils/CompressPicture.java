...
public class CompressPicture {
	...
	private static void compressImage(Bitmap image,File file) {
		...
		try {
			...
			image.compress(Bitmap.CompressFormat.JPEG, 80, b);//质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中
			int options = 80;
			/*while ( baos.toByteArray().length / 1024>100) {	//循环判断如果压缩后图片是否大于100kb,大于继续压缩		
				b.reset();//重置baos即清空baos
				image.compress(Bitmap.CompressFormat.JPEG, options, b);//这里压缩options%，把压缩后的数据存放到baos中
				options -= 10;//每次都减少10
			}*/
